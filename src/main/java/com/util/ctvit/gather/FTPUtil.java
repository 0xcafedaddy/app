...
public class FTPUtil {
	 ...
	 */
	public static String upload(FTPClient ftpClient, String materialId, String localFile) {
		try {
			...
			if (ftpFiles != null && ftpFiles.length > 0) {
				for (FTPFile ftpFile : ftpFiles) {
					...
					if (ftpFile.getName().equals(fileName)) {
						...
						// 尝试移动文件内读取指针,实现断点续传
						result = uploadFile(materialId, remoteFileName, file, ftpClient, remoteSize);
				...
				result = uploadFile(materialId, remoteFileName, file, ftpClient, 0);
				logger.debug("上传结果：" + result);
	...
	public static String uploadFile(String materialId, String remoteFile, File localFile, FTPClient ftpClient, long remoteSize) throws IOException {
		String status;
		// 显示进度的上传
		...
		int c;
		//Map<String, Object> map = (Map<String, Object>) CloudConstants.ftpUploadHashMap.get(materialId);
		while ((c = raf.read(bytes)) != -1) {
			out.write(bytes, 0, c);
			localreadbytes += c;
			//map.put(localFile.getAbsolutePath(), localreadbytes);
		}
		out.flush();
		raf.close();
		out.close();
	...
	public static void main(String[] args) throws IOException {
		...
		String status = FTPUtil.upload(ftpClient, "D:/temp/电影天堂www.dygod.net.三十极夜.中英双字.1024分辨率.rmvb", null);
