...
import com.uflowertv.model.vo.XxjHomeVO;
...
import static com.util.commons.ConstantHolder.HOME_VIDEO_LIST;
import static com.util.commons.ConstantHolder.NOTICE;
...
public class ProductionResource {
	...
	public Map<String,Object> home(@PathParam(value = "platformId") int platformId) {
        ...
        ArrayList<XxjHomeVO> list = Lists.newArrayList();
        List<XxjHomeDTO> homeJsons = xxjProductionService.getHomeJson(platformId);
        for (XxjHomeDTO xxjHomeDTO : homeJsons){
            XxjHomeVO xxjHomeVO = new XxjHomeVO();
            BeanUtilsApache.copyProperties(xxjHomeVO,xxjHomeDTO);
            list.add(xxjHomeVO);
        }
        map.put("code", 200);
		map.put("data", list);
