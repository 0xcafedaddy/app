...
public class PlUploadServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String datefolder = "/" + new DateTime().toString("yyyy") + "/" + new DateTime().toString("MM") + "/" + new DateTime().toString("dd");// 日期命名的文件夹
		String webParentPath = new File(request.getSession().getServletContext().getRealPath("/")).getParent();// 当前WEB环境的上层目录
		String realPath = webParentPath + ConstantConfig.getProperty("uploadPath") + fileFolder + datefolder;// 文件上传到服务器的真实路径
		// System.out.println(realPath);
		String path = ConstantConfig.getProperty("uploadPath") + fileFolder + datefolder;// 文件在服务器的相对路径
		// System.out.println(path);
		if (request.getSession() == null || request.getSession().getAttribute(ConstantConfig.getProperty("sessionInfoName")) == null) {// 如果用户没有登录，则不允许上传
