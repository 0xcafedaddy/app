...
public class PHPPackageServiceImpl implements PHPPackageServiceI{
	...
	public Map<String, Object> getSpecialInfo(int product_id){
		...
		CommonsEntityJson specialEntity = checkNotNull(RedisEntity.getEntity(String.format(SPECIAL_OBJ, PLATFORM_ID,special_id)),"专题"+special_id+"不存在");
	...
	public Map<String, Object> getSynCourseInfo(int product_id) {
		...
		while(it.hasNext()){
			...
			entityJson.setTerms(getTerms(vids,type));
	...
	@SuppressWarnings("rawtypes")
	private List<CommonsEntityJson> getTerms(List<Integer> vids,int type){
		List<CommonsEntityJson> terms = Lists.newArrayList();
		Map<String, Object> params = Maps.newHashMap();
		...
		for (int i = 0; i < list.size(); i++) {
			...
			switch (term) {
				case 0:
					CommonsEntityJson json1 = CommonsEntityJson.getInstance();
					json1.setTerm("全部");
					json1.setPoints(getPointList(vids,term,type));
					terms.add(json1);
					break;
				case 1:
					CommonsEntityJson json2 = CommonsEntityJson.getInstance();
					json2.setId(1);
					json2.setTerm("上学期");
					json2.setPoints(getPointList(vids, term,type));
					terms.add(json2);
					break;
				case 2:
					CommonsEntityJson json3 = CommonsEntityJson.getInstance();
					json3.setId(2);
					json3.setTerm("下学期");
					json3.setPoints(getPointList(vids, term,type));
					terms.add(json3);
					break;
				default:
					break;
	...
	@SuppressWarnings("rawtypes")
	private List<CommonsEntityJson> getPointList(List<Integer> vids,int term,int type){
		List<CommonsEntityJson> entityJsons = Lists.newArrayList();
		Map<String, Object> params = Maps.newHashMap();
		params.put("video_list", vids);
		params.put("video_status", VIDEO_ENABLE);
		...
		for (int i = 0; i < pointList.size(); i++) {
			...
			entityJson.setVideos(getVideoList(vids,term,point_id,type));
			entityJsons.add(entityJson);
		}
		return entityJsons;
	}
	
	private List<CommonsEntityJson> getVideoList(List<Integer> vids,int term,int point_id,int type){
		Map<String,Object> params = Maps.newHashMap();
		List<CommonsEntityJson> list = null;
		params.put("term", term);
		params.put("point_id", point_id);
		params.put("video_status", VIDEO_ENABLE);
		...
		switch (type) {
			case 1://同步课视频
				sql = "SELECT video_id,video_name,is_free,courseware_count FROM xxj_video WHERE video_status =:video_status AND term=:term AND point_id=:point_id ORDER BY sort";
				list = getPublicVideoList(xxjVideoDao.findBySql(sql, params));
				break;
			case 2://教师所教视频
				params.put("video_list", vids);
				list = getPublicVideoList(xxjVideoDao.findBySql(sql, params));
	...
	public Map<String,Object> getTeacherSubjectInfo(int teacher_id){
		...
		json2List.forEach(commonsEntityJson->{
			...
			for (int i = 0; i < list.size(); i++) {
				...
				gradeEntity.setSubjects(getSubjectList(vids,type));
	...
	@SuppressWarnings("rawtypes")
	private List<CommonsEntityJson> getSubjectList(List<Integer> vids,int type) {
		List<CommonsEntityJson> entityJsons = Lists.newArrayList();
		Map<String,Object> params = Maps.newHashMap();
		params.put("video_list", vids);
		params.put("video_status", VIDEO_ENABLE);
		...
		for (int i = 0; i < list.size(); i++) {
			...
			subjectEntity.setTerms(getTerms(vids,type));
