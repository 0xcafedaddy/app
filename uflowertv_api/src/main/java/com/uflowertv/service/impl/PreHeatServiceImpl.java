...
public class PreHeatServiceImpl implements PreHeatServiceI{
		...
		try {
			...
			xxjGrades.forEach(grade->{
				...
				xxjProducts.forEach(product->{
					...
					//产品详情
					phpPackageService.getSynCourseInfo(id);
				});
			});
			
			//专题对象
			List<XxjSpecial> xxjSpecials = xxjSpecialDao.find("from XxjSpecial");
			xxjSpecials.forEach(xxjSpecial->{
				String specialKey = String.format(SPECIAL_OBJ, PLATFORM_ID,xxjSpecial.getSpecial_id());
				p.expire(specialKey,0);
				Map<String, String> describe = BeanUtilsApache.describe(xxjSpecial);
				p.hmset(specialKey, describe);
			});
			...
			specials.forEach(product->{
				...
				//专题详情
				log.info("获取专题详情");
				phpPackageService.getSpecialInfo(id);
			});
			p.sync();
		} catch (Exception e) {
			e.printStackTrace();	
	...
	/**
	 * 教师下科目详情
	 * @see com.uflowertv.service.PreHeatServiceI#getTeacherSubjectInfoRelation()
	 */
	@Override
	public void getTeacherSubjectInfoRelation(){
		//jedis
		Jedis jedis = null;
		try {
			jedis = RedisUtil.getJedis();
		...
		try {
			...
			xuedList.forEach(xued_obj->{
				...
				gradeList.forEach(grade_obj->{
					...
					gradeEntity.setSubjects(getSubjectList(Integer.valueOf(grade_id)));
	...
	private List<CommonsEntityJson> getSubjectList(int grade_id) {
		...
		try {
			...
			Response<List<String>> response = p.sort(String.format(GRADE_SUBJECT_LIST, grade_id),params);
			p.sync();
			List<String> subjectList = response.get();
	...
	private List<CommonsEntityJson> getFreeVideoList(int subject_id) {
		List<CommonsEntityJson> videoList = Lists.newArrayList();
		Map<String, Object> params = Maps.newHashMap();
		params.put("is_free", VIDEO_FREE);
		params.put("video_status", VIDEO_ENABLE);
		params.put("subject_id", subject_id);
		String hql = "FROM XxjVideo WHERE video_status =:video_status AND is_free = :is_free AND subject_id = :subject_id ORDER BY sort";
		List<XxjVideo> xxjVideos = xxjVideoDao.find(hql, params);
		if(CollectionUtils.isEmpty(xxjVideos)){
			return Collections.emptyList();
		}
		xxjVideos.forEach(xxjVideo->{
			CommonsEntityJson entityJson = CommonsEntityJson.getInstance();
			BeanUtilsApache.copyProperties(entityJson, xxjVideo);
			videoList.add(entityJson);
		});
		return videoList;
	}
}
