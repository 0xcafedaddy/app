...
public class HttpClientUtils {
	...
	public static String uploadPic(String url,String path){
		CloseableHttpResponse httpResponse = null;
		HttpPost httpPost = new HttpPost(url);  
		try {  
		    HttpEntity req = MultipartEntityBuilder.create()  
		            .setMode(HttpMultipartMode.BROWSER_COMPATIBLE)  
		            .addPart("files", new FileBody(new File(path)))  
		            .build();  
		    ...
		    if(httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK){
		    	...
		    	String response = EntityUtils.toString(re, "UTF-8");
				return response;
		    }
		} catch (IOException e) {
			e.printStackTrace();
	...
	public static String upload(String remoteUrl,String localPath){
		CloseableHttpResponse httpResponse = null;
		...
		try {
			...
			HttpEntity reqEntity = MultipartEntityBuilder.create().
					setMode(HttpMultipartMode.BROWSER_COMPATIBLE).
					addPart("uploadFile", new FileBody(localFile)).
