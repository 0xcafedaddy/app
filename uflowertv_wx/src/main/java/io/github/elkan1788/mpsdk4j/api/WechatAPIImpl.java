...
public class WechatAPIImpl implements WechatAPI {
    ...
    @Override
    public File dlMedia(String mediaId,String path) {
        String url = mergeCgiBinUrl(get_media, getAccessToken(), mediaId);
        ApiResult ar = null;
        for (int i = 0; i < RETRY_COUNT; i++) {
          //  Object tmp = HttpTool.download(url,path);
        	  Object tmp = HttpTool.downloadRemote(url,path);
    ...
    /**
     * 文本消息
     * @see MessageAPI#sendCoustomText(String, String)
     */
    @Override
	public String sendCoustomText(String openid,String content) {
    	String url = mergeCgiBinUrl(send_coustom + getAccessToken());
        ApiResult ar = null;
        TextMessage msg = new TextMessage();
  		TextMessage msg2 = new TextMessage(content);
        msg.setTouser(openid);
        msg.setMsgtype("text");
        msg.setText(msg2);
        String body = JsonUtils.bean2Json(msg);
        for (int i = 0; i < RETRY_COUNT; i++) {
            ar = ApiResult.create(HttpTool.post(url, body));
            if (ar.isSuccess()) {
                return String.valueOf(ar.get("errmsg"));
            }
        }
        throw Lang.wrapThrow(new WechatApiException(ar.getJson()));
	}
    /**
     * 多图文消息
     * @see MessageAPI#sendCoustomNews(String, List)
     */
    @Override
    public String sendCoustomNews(String openid,List<News> articles) {
    	String url = mergeCgiBinUrl(send_coustom + getAccessToken());
    	ApiResult ar = null;
    	NewsMessage msg = new NewsMessage();
  		NewsMessage msg2 = new NewsMessage();
  		msg.setTouser(openid);
  		msg.setMsgtype("news");
  		msg2.setArticles(articles);
  		msg.setNews(msg2);
        String body = JsonUtils.bean2Json(msg);
    	for (int i = 0; i < RETRY_COUNT; i++) {
    		ar = ApiResult.create(HttpTool.post(url, body));
